...
//	Used primarily to store hashes as integers mod L:
	...
	bool greaterThanOrEqualToOrder(const uint32_t*);
...
inline void Ed25519SignatureAlgorithm::generateReadAndPruneHash(const unsigned char* privateKey) {
...
inline void Ed25519SignatureAlgorithm::hashModOrder(uint32_t* intOut, const unsigned char* message, const unsigned long long messageBytes) {
...
inline bool Ed25519SignatureAlgorithm::greaterThanOrEqualToP(const uint32_t* a) {
	for(unsigned short i = 0; i < INT_LENGTH_MULTI; i += 1) {
		if(a[i] > p[i]) {
			return true;
		}
...
inline bool Ed25519SignatureAlgorithm::equalToZero(const uint32_t* a) {
	for(unsigned short i = 0; i < INT_LENGTH_MULTI; i += 1) {
		if(a[i] != 0x00000000) {
			return false;
		}
...
inline bool Ed25519SignatureAlgorithm::decodePoint(uint32_t* pointOutX, uint32_t* pointOutY, uint32_t* pointOutZ, uint32_t* pointOutT, const unsigned char* encodedPoint) {
	for(unsigned short i = 0; i < INT_LENGTH_MULTI; i += 1) {
...
inline bool Ed25519SignatureAlgorithm::greaterThanOrEqualToOrder(const uint32_t* a) {
	for(unsigned short i = 0; i < INT_LENGTH_MULTI; i += 1) {
		if(a[i] > L[i]) {
			return true;
		}
...
inline void Ed25519SignatureAlgorithm::initialize(unsigned char* publicKeyOut, const unsigned char* privateKey) {
...
inline void Ed25519SignatureAlgorithm::sign(unsigned char* signatureOut, unsigned char* publicKeyInOut, const unsigned char* privateKey, const unsigned char* message, const bool createPublicKey, const unsigned long long messageBytes = KEY_BYTES) {
...
inline bool Ed25519SignatureAlgorithm::verify(const unsigned char* publicKey, const unsigned char* message, const unsigned char* signature, const unsigned long long messageBytes = KEY_BYTES) { // Not constant time: all components are public.
